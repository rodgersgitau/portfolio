  ...
  return (
    ...
    >
      <Section className="w-full my-8">
        <motion.div
          variants={riseWithFade}
          className="flex flex-col-reverse items-center w-full gap-40 py-4 lg:flex-row"
        >
          <div className="w-full lg:w-[50%] h-full flex flex-col gap-8 ">
            <h1 className="flex items-center gap-4 font-semibold">
              ...
              <span className="relative inline-block align-middle p-[4px] before:h-[105%] before:absolute before:-inset-1 before:top-[50%] before:-skew-y-3 before:-translate-y-[50%] before:bg-black before:dark:bg-pink-500 before:rounded-md">
                <span className="relative mx-auto text-2xl text-white">
                  <AnimatedWords title="Rodgers M Gitau" />
                </span>
              </span>
            </h1>
            <div className="flex flex-col gap-8 text-lg text-slate-800 dark:text-slate-400">
              <p>
                ...
                the past 7+ years, I've designed, developed & shipped all sorts
                of web applications for a myriad of industries i.e hospitality,
                real estate, ecommerce & healthcare
              </p>
              <p>
      ...
      <Section title="projects" className="w-full mx-auto my-8">
        <motion.div
          ...
          className="grid w-full h-full grid-cols-1 gap-20 mx-auto my-10 max-w-7xl lg:grid-cols-2"
        >
          {projects.map((project, idx) => (
            <motion.div
              key={project.slug}
              variants={dropWithFade}
              className={`relative w-full h-full group ease-in-out ${
                idx % 2 === 1
                  ? "top-[15%] -translate-y-[15%]"
                  : "top-0 translate-y-0"
              }`}
            >
              <div
              ...
              />
              <div
                className={`relative flex flex-col justify-center min-h-[35rem] gap-8 p-8 rounded-lg bg-slate-100 text-black/70 dark:text-slate-100 dark:bg-black/90 group-hover:dark:bg-black group-hover:bg-slate-100 ring-slate-400/5 ring-1 dark:ring-slate-900/5`}
              >
                <div className="relative flex-1 overflow-hidden rounded-t-lg shadow aspect-video min-h-[15rem] shadow-current/90 lg:rounded-lg">
                  <NextImage
                    alt=""
                    fill
