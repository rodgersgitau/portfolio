...
const FormId = process.env.NEXT_PUBLIC_FORMSPREE_ID || process.env.NEXT_PUBLIC_FORM;
		...
		<div className="h-full w-90 md:w-[45vw] mx-auto flex flex-col gap-4">
			...
			>
				...
				{inputs.map(({ name, label, required, ...rest }) => {
					return (
						...
						>
							...
							/>
							<label htmlFor={name} className="text-sm font-semibold peer-focus:text-pink-500">
				...
				<div className="relative z-0 flex flex-col-reverse w-full col-span-2 gap-3 text-sm text-black group dark:text-gray-400">
					...
					/>
					<label htmlFor="message" className="text-sm font-semibold peer-focus:text-pink-500">
						Your Message
					</label>
				</div>
				<button
			...
			<small className="text-xs text-right text-black dark:text-gray-500">* Required Fields</small>
		</div>
	);
};
